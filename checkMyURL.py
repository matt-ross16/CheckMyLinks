    ...
    try:
        with open(filepath, "r") as file_object:
            for link in bs4.BeautifulSoup(
                file_object.read(), "html.parser", parse_only=bs4.SoupStrainer("a")
            ):
                linkIgnored = False
                if ignoreLinks:
                    for iLink in ignoreLinks:
                        if len(re.findall(iLink, str(link))) != 0:
                            linkIgnored = True
                if link.has_attr("href") and linkIgnored is False:
    ...
    try:
        if link_checker(base_link, link_filter)[1] == 200:
            url_request = urllib.request.Request(
                base_link, headers={"User-Agent": "Magic-Browser"}
            )
...
@click.option("-i", "--ignore", type=str, help="Use file to filter out unwanted URLS")
@click.option(
    "-t",
    "--telescope",
    "telescope",
...
def main(parse_link, parse_file, save_file, link_filter, ignore, telescope, link):
    ...
    Note: if multiple filters are specified, only the last one entered will be applied.
    """
    click.clear()
    if parse_link:
